...
class UAVControl:
    ...
    def reboot(self):  # left synchronous
        pass
    ...
    async def std_send(self, msg_code: int, data=[]):
        """Asynchronous wrapper around send_RAW_msg, receive_msg, process_recv_data."""
        sent = await self.run_sync(self.board.send_RAW_msg, msg_code, data, None, None, False)
        if sent:
            dataHandler = await self.run_sync(self.board.receive_msg)
            await self.run_sync(self.board.process_recv_data, dataHandler)
            return dataHandler
        else:
            return None
    ...
    def load_modes_config(self):
        ...
        for i in mranges:
            ...
            valmin = 900+(i[2]*25)
            valmax = 900+(i[3]*25)
            or_flag = "OVERRIDE ALLOWED" if ch in self.msp_override_channels else ""
            print(f"ID: {i[0]}\t{modename:<16s}:\t{auxn} (channel {ch})\t= {valmin} to {valmax}\t{or_flag}")
    ...
    def arm_enable_check(self):
        ...
        valid_flags = [i for i in arm_d_flags if i not in (
    ...
    def arm(self):
        arm_d_flags = self.board.process_armingDisableFlags(self.board.CONFIG['armingDisableFlags'])
        check = (not arm_d_flags) or arm_d_flags == [inavutil.armingDisableFlagNames_INAV.SIMULATOR_MODE]
    ...
    def set_mode(self, mode: int, on: bool) -> None:
        ...
        pwm = (sum(pwm_range)//2) if on else 900
        self.channels[channel-1] = pwm
        print(f'mode switch {inavutil.modesID_INAV.get(mode)} to {on} ch {channel-1} to {pwm}')

    def get_active_modes(self) -> list:
        active = []
        for mode, (channel, pwm_range) in self.modes.items():
            if self.channels[channel] == sum(pwm_range)//2:
                active.append(mode)
        return active

    def get_board_modes(self):
        if not self.run:
            self.std_send(inavutil.msp.MSP2_INAV_STATUS)
    ...
    def set_rc_channel(self, ch:str, value:int):
        idx = self.chorder.index(ch)
        if self.msp_receiver or (self.msp_override and idx+1 in self.msp_override_channels):
    ...
    def get_sensor_config(self):
        if not self.run:
            self.std_send(inavutil.msp.MSP_SENSOR_CONFIG)
        self.sensors = {
    ...
    def get_imu(self):
        if not self.run:
            self.std_send(inavutil.msp.MSP_RAW_IMU)
        self.imu = {
    ...
    def get_analog(self):
        if not self.run:
            self.std_send(inavutil.msp.MSP2_INAV_ANALOG)
    ...
    def set_wp(self, wp_no, action, lat, lon, altitude, p1, p2, p3, flag):
        msp_wp = self.pack_msp_wp(int(wp_no), int(action),
        ...
        if ret and ret['dataView']:
            ...
            return geospatial.Waypoint(b[0], b[1],
    ...
    def _set_wp(self, wp):
        return self.set_wp(wp.wp_no, wp.action,
                           wp.pos.lat, wp.pos.lon, wp.pos.alt,
                           wp.p1, wp.p2, wp.p3, wp.flag)
    ...
    def get_wp_info(self):
        ret = self.std_send(inavutil.msp.MSP_WP_GETINFO)
        if ret:
    ...
    async def flight_loop(self):
        """Async flight control loop; all blocking board ops run in executor."""
        rc_interval_t = 0
        await self.telemetry_init()
        if not self.board.INAV:
            raise Exception('Non-INAV board detected')

        # load modes in executor
        ...
        try:
            while self.run:
                ...
                if (time.time() - rc_interval_t) >= (1.0 / self.rc_interval) and (self.msp_receiver or self.msp_override_active):
                    ...
                    if await self.run_sync(self.board.send_RAW_RC, self.channels):
